...
autoplot(rf_tune, metric='rmse') +
  ...
  theme_polyvore()
```
The autoplot for the random forest tells us that 100 trees with mtry=4 seems to have the best performance. This makes sense, as having more trees gives the model more flexibility which would be helpful in the case where there are a lot of predictors and categories within those predictors.
...
We can see in the boosted tree metrics that the learning rate of 0.1 worked the best by far, and having 100 trees was also the best, though the change between the number of trees is not very dramatic. This indicates that a faster learning rate was beneficial for the model.
