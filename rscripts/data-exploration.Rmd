...
train_json <- jsonlite::fromJSON('./polyvore_data/train_no_dup.json', flatten=TRUE, simplifyDataFrame = TRUE)
...
```{r}
length(train_df$items$index) <- 8
new_df <- do.call(data.frame, train_df)
```
